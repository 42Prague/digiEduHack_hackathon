...
# ====================================================================
# Step 3: Load reference tables (demographics)
# ====================================================================

echo "📋 Step 3: Checking reference tables..."
# Check if reference tables exist
REF_TABLES_EXIST=$(sqlite3 "$DB_PATH" "SELECT name FROM sqlite_master WHERE type='table' AND name='ref_regions';" 2>/dev/null || echo "")

if [ -z "$REF_TABLES_EXIST" ]; then
    echo "   Reference tables not found. Loading demographics..."
    
    # Set database path for the script
    export DB_PATH="$DB_PATH"
    
    # Run demographics loader
    if python3 load_demographics.py > /dev/null 2>&1; then
        echo "   ✅ Reference tables loaded successfully"
    else
        echo "   ⚠️  Warning: Failed to load reference tables"
        echo "   Some dropdown menus may not work properly"
    fi
else
    echo "   ✅ Reference tables already exist"
fi
echo ""

# ====================================================================
# Step 4: Generate mock data if requested
# ====================================================================

echo "📊 Step 4: Checking mock data..."
...
echo "🔍 Step 5: Verifying database health..."
