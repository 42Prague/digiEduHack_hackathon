  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">Vítejte zpět, <span className="font-semibold">{session.user.name}</span>!</p>
      </div>
      
      {/* Admin Info Card */}
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">Váš administrátorský profil</h2>
        <div className="p-4 bg-blue-50 rounded-lg">
          <ul className="space-y-2 text-sm text-gray-700">
            <li><span className="font-semibold">Email:</span> {session.user.email}</li>
            <li><span className="font-semibold">Role:</span> <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded">Administrátor</span></li>
            <li><span className="font-semibold">ID uživatele:</span> {session.user.id}</li>
          </ul>
        </div>
      </div>
