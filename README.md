...
Need a quick summary of raw text (under ~1000 tokens)? The FastAPI service now exposes `POST /summarize`:
...
The response contains a single-paragraph summary generated by the configured LLM, along with metadata about the model and token usage. The endpoint enforces a lightweight length check and returns `400` if the payload is empty or too large for a single request.

### Text embedding endpoint

Need a compact semantic vector? Use the CPU-friendly `POST /embed` route backed by the permissively licensed [`BAAI/bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5) model (MIT license):
...
{
	"text": "Skills-based grading policies increase equity."
}
```

The server returns a JSON payload with the normalized embedding vector and the model name. The sentence-transformer runs entirely on CPU, so no GPU is required for development or deployment.

> ℹ️ **CPU-only installs**: `requirements.txt` includes `--extra-index-url https://download.pytorch.org/whl/cpu`, which tells pip to prefer the lightweight CPU wheels for PyTorch/transformers. PyTorch 2.2 still links against NumPy 1.x, so the file pins `numpy==1.26.4` to avoid the runtime crash you get with NumPy 2.x. No CUDA toolkits are downloaded during `pip install -r requirements.txt`.